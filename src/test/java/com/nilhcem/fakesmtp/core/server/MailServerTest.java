...
public class MailServerTest {
	...
	public void testSaveDeleteEmail() throws UnsupportedEncodingException, InterruptedException {
		...
		Observer mockObserver = new Observer() {
			...
			public void update(Observable o, Object arg) {
				...
				filesModel.setEmlFilePath(model.getFilePath());
				filesModel.setHtmlFilePath(model.getHtmlFilePath());

				UIModel.INSTANCE.getListMailsMap().put(0, filesModel);
				saver.deleteEmails();
	...
	private String getMockEmail(String from, String to, String subject, String content) {
		...
		StringBuilder sb = new StringBuilder().append("Line 1 will be removed").append(br).append("Line 2 will be removed").append(br).append("Line 3 will be removed").append(br).append("Line 4 will be removed").append(br).append(
				"Date: Thu, 15 May 2042 04:42:42 +0800 (CST)").append(br).append(String.format("From: \"%s\" <%s>%n", from, from)).append(String.format("To: \"%s\" <%s>%n", to, to)).append(
				"Message-ID: <17000042.0.1300000000042.JavaMail.wtf@OMG00042>").append(br).append(String.format("Subject: %s%n", subject)).append("MIME-Version: 1.0").append(br).append("Content-Type: text/plain; charset=us-ascii").append(
				br).append("Content-Transfer-Encoding: 7bit").append(br).append(br).append(content).append(br).append(br);
		return sb.toString();
	}
